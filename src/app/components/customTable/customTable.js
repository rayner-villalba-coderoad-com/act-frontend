(function () {
  var customTable;
  try {
    customTable = angular.module("customTable")
  } catch (err) {
    customTable = null;
  }

  if (!customTable) {
    angular.module('customTable.services', ['ngAnimate']);
    angular.module('customTable.controllers', []);
    angular.module('customTable.directives', []);
    angular.module('customTable.constants', []);
    angular.module('customTable',
      [
        'customTable.services',
        'customTable.controllers',
        'customTable.directives',
        'customTable.constants'
      ]);
  }
  ...
  var customTableController = function ($scope, $attrs, $parse, $timeout, $animate, $log, $window) {
    var tblCtrl = this,
      ...
      initDelegates = function () {
        ...
        $tbody.on('click', 'td.td-checkbox > input[type="checkbox"]', function (e) {
          ...
          if (tblCtrl.singleSelect) {
            $scope.pagedData.forEach(function (tempModel) {
              if (tempModel != model) {
          ...
          if($scope.options.callbacks && $scope.options.callbacks.rowClicked) {
            $scope.options.callbacks.rowClicked({selectedRow : model });
          }
      ...
      redrawTable = function () { console.log('redrawTable');
        start = new Date().getTime();
        ...
        if ($scope.options.updatedRecords) {
          if ($scope.options.updatedRecords.length > 10) {
            ...
            var $rows = [],
              ...
              indexRegex = /([\d]+)/g;
            $scope.options.updatedRecords.forEach(function (model) {
              var key = model.id;
...
*/
    ...
    return {
      ...
      link: function (scope, element, attrs, ctrls) {
        ...
        var getRepeatTable = function () {
          ...
          //Set the defined columns
          var columnsDefinitions = scope.options.columnDefns;

          columnsDefinitions.forEach(function(column, index) {
            tableColumn = column;
  ...
  angular.module("customTable").run(["$templateCache", function ($templateCache) {
    $templateCache.put("template/pagination/customPagination.html",
      "<div class=\"pagination\">\n"+
      "    <button ng-if=\"boundaryLinks\" ng-class=\"{disabled: noPrevious()}\" ng-click=\"selectPage(1)\">{{getText('first')}}</button>"+
      "    <button ng-if=\"directionLinks\" ng-class=\"{disabled: noPrevious()}\" ng-click=\"selectPage(page - 1)\">{{getText('previous')}}</button>"+
      "    <input type=\"text\"  min=\"1\" max=\"5\" >" +
      "    <button ng-if=\"directionLinks\" ng-class=\"{disabled: noNext()}\" ng-click=\"selectPage(page + 1)\">{{getText('next')}}</button>" +
      "    <button ng-if=\"boundaryLinks\" ng-class=\"{disabled: noNext()}\"  ng-click=\"selectPage(totalPages)\">{{getText('last')}}</button>" +
      "</div>"
    );
    ...
    $templateCache.put("template/table/customTableRow.html",
      ...
      "<tr ng-repeat='<--RECORD--> in pagedData track by <--RECORDKEY-->'  ng-click=\"tblCtrl.selectionChanged(<--RECORD-->)\" ng-class=\"<--COMPUTEDCLASS-->\" <--REPEATFINISH-->>\n" +
      "  <td class=\"td-checkbox\"><input type=\"checkbox\" ng-model=\"<--RECORD-->.isSelected\" /></td>\n" +
      "  <--CELLS-->\n" +
      "</tr>");

    $templateCache.put("template/table/customTableRowNoRepeat.html",
      "<tr data-key=\"<--RECORDKEY-->\" data-model=\"<--RECORD-->\" ng-click=\"tblCtrl.selectionChanged(<--RECORD-->)\" class=\"<--COMPUTEDCLASS-->\">\n" +
      "  <td class=\"td-checkbox\"><input type=\"checkbox\" ng-model=\"<--RECORD-->.isSelected\" /></td>\n" +
      "  <--CELLS-->\n" +
      "</tr>");
  ...
  angular
    .module('customTable.controllers')
